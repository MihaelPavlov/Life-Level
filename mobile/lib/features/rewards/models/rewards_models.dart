...
class TaskRewardPeriod {
  ...
  bool get hasClaimableTask =>
      tasks.any((task) => task.isCompleted && !task.rewardClaimed);

  bool get hasClaimableMilestone => milestones
      .any((milestone) => milestone.isUnlocked && !milestone.isClaimed);

  bool get hasClaimableReward => hasClaimableTask || hasClaimableMilestone;
}
...
class TaskRewardsClaimResult {
  final String period;
  final int tasksClaimed;
  final int coins;
  final int crystals;
  final TaskRewardPeriod updatedPeriod;

  const TaskRewardsClaimResult({
    required this.period,
    required this.tasksClaimed,
    required this.coins,
    required this.crystals,
    required this.updatedPeriod,
  });

  factory TaskRewardsClaimResult.fromJson(Map<String, dynamic> j) =>
      TaskRewardsClaimResult(
        period: j['period'] as String,
        tasksClaimed: (j['tasksClaimed'] as num).toInt(),
        coins: (j['coins'] as num).toInt(),
        crystals: (j['crystals'] as num).toInt(),
...
class RewardCenterData {
  ...
  const RewardCenterData(
      {required this.wallet, required this.daily, required this.weekly});
  ...
  bool get hasClaimableReward =>
      daily.hasClaimableReward || weekly.hasClaimableReward;
}
