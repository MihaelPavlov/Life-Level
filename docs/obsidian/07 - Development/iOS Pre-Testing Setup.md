...
> [!warning] **Status: repo prep started; account-side setup still pending.** Android testing is fully unblocked. iOS still requires Mac/Xcode, Apple Developer Program access, App Store Connect, and Firebase iOS/APNs setup.
...
### Task 1 — iOS bundle ID
**Chosen:** `com.lifelevel.app`

Use this exact Bundle ID in Apple Developer, Firebase iOS, Xcode signing, and App Store Connect. Do not register the old scaffold ID `com.example.lifeLevel`.

Repo-side status: `mobile/ios/Runner.xcodeproj/project.pbxproj` now uses `com.lifelevel.app` for the Runner target.
...
3. Bundle ID: explicit, `com.lifelevel.app`
...
2. Project Settings → General → Add app → iOS → Bundle ID `com.lifelevel.app`
...
> Also re-run `flutterfire configure` for iOS after creating the new Firebase iOS app so `lib/firebase_options.dart` gets the real iOS `appId` for `com.lifelevel.app`.
...
| File | Source | Notes |
|---|---|---|
| `mobile/ios/Runner/GoogleService-Info.plist` | Firebase Console download | NOT secret — safe to commit; currently missing |
| `mobile/ios/Runner/Runner.entitlements` | Repo-prepped + Xcode capability verification | Commit |
| `mobile/ios/Runner.xcodeproj/project.pbxproj` | Repo-prepped + Xcode signing/team edits | Commit |
| `mobile/ios/Runner/Info.plist` | HealthKit usage descriptions + remote notification background mode | Verify still present |
